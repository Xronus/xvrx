...
<div class="row"><div class="col-12"><div class="card"><div class="card-body">
    <table class="table table-bordered table-hover mb-0">
        <thead class="table-light"><tr><th>ID</th><th>{{ __('main.name') }}</th><th>{{ __('main.parent_category') }}</th><th>{{ __('main.sort') }}</th><th></th></tr></thead>
        <tbody>
            @forelse($categories as $cat)
            <tr>
                <td>{{ $cat->id }}</td>
                <td>{{ $cat->localizedName() }}</td>
                <td>{{ $cat->parent?->localizedName() ?? '—' }}</td>
            ...
            @empty
            <tr><td colspan="5" class="text-muted text-center py-4">{{ __('main.shop_no_categories') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div></div></div></div>
